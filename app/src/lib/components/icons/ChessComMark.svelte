<script>
  /**
   * Chess.com source mark — §3.2.3.3.
   *
   * Path from Simple Icons (CC0 1.0), the pawn mark; the brand’s green tile is not part of the
   * monochrome mark, so the silhouette carries the identity here.
   * CC0 covers the artwork data; the mark itself remains Chess.com’s trademark.
   *
   * Filled, not stroked, so it sits beside Lucide’s stroked icons without
   * matching their weight — which is the point: a source mark should read as
   * a brand, not as another piece of interface furniture.
   */
  let { size = 16, label = null } = $props();
</script>
